...
| M3 | ✅ 已确认 2026-08-31 | ✅ 已确认 2026-08-31 | ✅ 已确认 2026-08-31 | ✅ 完成 2026-08-29 | T1~T7 全部实现并验收（317 测试）：独立 potential_relationships 表（a<b 唯一键+方法枚举+confidence CHECK 三保险）；两方法纯派生计算 common_network=43 / research_similarity=1111（零 GLM 零 token，NFR-1）；周日 06:00 单事务全量重算（幂等/回滚保旧全量，1.4s ≪5min，NFR-2）；C11 巡检并入 C1-C11 全绿；API include_potential（默认关）+ 人详情 potential_connections；前端虚线灰边+开关+潜在关系面板+潜在连接段（浏览器走查全绿） |
| M4 | — | — | — | — | 未开始 |
