...
> **阶段**：Plan（plan.md）✅ 已确认（2026-08-27，引导式 review：OQ-1~3 逐条决议 + 种子清单确认）
> **状态**：定稿
...
│  图谱页：三类型边样式统一（RD-M2-13）——hover/点击显示类型+子类型 + 类型筛选器     │
...
**多音字策略**：pypinyin 默认常用音；归一失配的落入打分或队列，由审核兜底，不做姓名字典。`find_candidates` 候选集扩展：拼音归一精确 + 现有编辑距离模糊。【2026-08-29 修订，M1 spec §9-2：编辑距离模糊已取消，候选=归一精确（含颠倒序）】
...
crawl:
  ...
  seeds:
    ...
    # 可选第三种子已议弃（2026-08-27 用户确认 NISL+IPADS 两家）：北大 NEEC 师资页
    # 只有教师无学生名单，对传承弱，仅对 FR-3.6 字段补全有用，暂不纳入。
```
...
frontend/src/
├── components/GraphCanvas  # 统一边样式（RD-M2-13）；边 hover tooltip + 点击显类型/子类型
...
1. **高校种子入口 URL**（RD-M2-9 遗留）→ **已调研、实测验证并经用户确认定稿：NISL + IPADS 两种子**（§8）：
   ...
   - ⚪ 北大 NEEC 师资页（静态 .htm，仅教师无学生名单）——用户确认弃用。
