...
> **阶段**：Tasks（tasks.md）✅ 已确认（2026-08-27，R1–R5 引导式 review）
> **状态**：定稿
...
- [x] **T10 资讯抽取 + projects 实体 news_extractor.py**
...
- [x] **T11 项目关系建立 project_linker.py**
...
- [x] **T14 前端：统一边样式 + 类型筛选器 + 证据面板**
  - 内容：GraphCanvas 三类型边**样式统一**（沿用 M1 实线，不做线型/颜色区分，RD-M2-13）；边 hover tooltip + 点击详情面板显示关系类型（学术传承含子类型）；FilterBar 增关系类型多选（默认全选）；EvidencePanel 混合证据列表（论文/网页快照/资讯三段，均可点击跳原文，网页证据展示快照时间）；学者详情增职位/主页字段。
  - 依赖：T13
  - 验证：开发服起来人工核对：混合展示不杂乱、点击边显示类型、筛选生效、证据链接可打开、详情字段显示。对应 AC-4/6（用户抽查）。

- [x] **T15 防护网扩展：双跑幂等 + 巡检验证 + 重筛优化**
