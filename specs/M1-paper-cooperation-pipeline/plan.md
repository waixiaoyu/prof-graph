...
**成本估算**：18 类日增约 500-700 篇；规则粗筛后待定约 30%；GLM 细筛约 150-200 篇/日 ≈ 15 万 tokens/日；抽取（AI 相关约 200-250 篇 × 2500 tokens）≈ 55 万 tokens/日，日常合计约 65-80 万。

**熔断设计（NFR-2 落实，2026-08-24 修订）**：
- **双预算**：日预算 **120 万 tokens** + 周预算 **600 万 tokens**（数值 .env 可配，T23 联调后按真实用量校准，不写死）。日常峰值约 80 万，日预算留 50% 余量，熔断回归"异常保护"而非"每日配给"。
- **分级熔断**：日预算达 80% 告警（后台横幅）→ 达 100% **先停细筛**（待定论文放行入库，宁多勿漏，粗筛兜底）→ 周Budget触顶或持续超耗时**停抽取**。规则粗筛和采集始终继续。
- **管理员覆盖**：任一级熔断触发后，管理员可在后台**手动决定继续**（当日放行，次日自动恢复自动模式）；操作记录进日志。
- 次日（或次周）自动恢复，无需人工干预。
...
新作者 → openalex_id 命中已有 Person？ → 是：归并
              ↓ 否
        5 因素打分：
          姓名 30%（归一精确一致（含姓名序颠倒）→ 1.0；否则 0.2
            【2026-08-29 修订，spec §9-2：原 编辑距离 ≥0.95→1.0 / 0.85–0.95→0.7 取消】）
