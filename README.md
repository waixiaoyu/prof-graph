...
**当前状态（2026-08-30）**：M1 论文合作全链路 + M2 学术传承与项目合作 + M2.5 后台手动编辑（人工治理闭环）已上线运行——每日自动链：备份 → 03:00 arXiv 采集管线（GLM 抽取）→ 04:00 RSS 资讯采集（项目合作证据）→ 05:00 高校官网爬取（学术传承证据，NISL/IPADS 种子）；图谱支持三类型关系筛选与混合证据（论文/网页快照/资讯）；管理端支持学者字段/机构/标签编辑、关系墓碑删除与强度校准、按人合规级联删除、操作日志审计（改后管线重跑不复活）；292 项测试、十项数据不变量巡检（C1–C10，墓碑兼容）、每日全库备份。2026-08-30 运行期加固：采集分类勘误 eess.IT（幻影分类）→ eess.IV 并加同实体双名守卫测试；GLM 全文输入统一拼 RSS 作者名单 + LaTeXML 转换失败残页识别回退（12 篇必然死信全部救回，死信 24→4）。M3 潜在关系挖掘待启动。进度详见 [specs/README.md](./specs/README.md)。
